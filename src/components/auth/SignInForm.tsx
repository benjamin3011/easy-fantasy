...
export default function SignInForm() {
  ...
  /* ────────────────────────── auth helpers ─────────────────────────────── */
  const applyPersistence = async (keep: boolean) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await applyPersistence(keepLoggedIn);
  ...
  const handleGoogleSignIn = async () => {
    setError(null);
    try {
      await applyPersistence(true); // OAUTH flows always persist locally
        ...
        <div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="space-y-6">
              ...
              {error && (
                <p className="text-sm text-error-500 -mt-4">{error}</p>
              )}
