...
async function saveAutoLineup(
  ...
  try {
    ...
    await lineupDocRef.set(lineupData);
    
    logger.info(`Merged auto-picks with existing lineup for user ${userId}: ${Object.keys(newPicks).length} new picks, ${Object.keys(mergedPicks).length} total picks`);
    
    // TODO: Also update usage counts (similar to saveWeeklyLineup logic)
...
async function applyAutoTipsForGames(
  ...
  try {
    ...
    // 1. Get existing weekly tips document
    const tipsDocId = `${leagueId}_${season}_${week}`;
    ...
    const currentTips = existingTips.exists ? existingTips.data()?.tips || {} : {};
    ...
    for (const game of games) {
      try {
        ...
        tipsApplied++;
        logger.info(`Auto-tip applied for game ${game.gameID}: ${favoriteTeam} (${tippableGame.homeWinProbability}% vs ${tippableGame.awayWinProbability}%)`);
